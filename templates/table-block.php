...
<!-- Уникальный контейнер для изоляции экземпляра шорткода -->
<div class="saintsmedia-table-block-wrapper" data-instance-id="<?php echo esc_attr($instance_id); ?>">
...
?>
    ...
    <script>
    (function() {
        // Получаем контейнер текущего экземпляра шорткода
        const wrapper = document.querySelector('[data-instance-id="<?php echo esc_js($instance_id); ?>"]');
        if (!wrapper) return;

        // == Нумерация карточек (только внутри текущего экземпляра) ==
        const orderNumbers = wrapper.querySelectorAll('.saintsmedia-order-number');
        orderNumbers.forEach((el, idx) => el.textContent = idx + 1);

        // == Обводка первой карточки (только в текущем экземпляре) ==
        const firstCard = wrapper.querySelector('.saintsmedia-casino-card--primary');
        if (firstCard) {
            firstCard.style.outline = '3.8px solid <?php echo esc_js($casino_table_outline); ?>';
        }

        // == Рейтинг и звёзды (только внутри текущего экземпляра) ==
        const ratings = wrapper.querySelectorAll('.what-is-rating');
...
</div><!-- /.saintsmedia-table-block-wrapper -->
